...
/*   Updated: 2023/01/12 00:06:13 by alevra           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	display_line(t_map *map, int line, t_win *win, t_p spacing);
...
static void	display_map(t_map *map, t_win *win)
{
	...
	while (i < map->height)
	{
		display_line(map, i, win, spacing);
		i ++;
	}
}
...
static void		display_line(t_map *map, int line, t_win *win, t_p spacing)
{
	...
	while (i < map->width)
	{
		...
		p.y = line_pos + offset.y;
		k = 0;
		if (i != map->width -1)
			while (k < spacing.x)
				put_pixel(p.x + k++, p.y, win);
		k = 0;
		if (line != map->height -1)
			while (k < spacing.y)
				put_pixel(p.x , p.y + k++, win);
